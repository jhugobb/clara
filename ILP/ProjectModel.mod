	...
	forall(b in B, s in S)
 	...
 	// Restrain the number of buses used
  	...
  	forall(s in S, b in B)
     	forall(s2 in S: startingTimeService[s] < startingTimeService[s2])
        	x_bs[b, s] * (startingTimeService[s] + durationTimeService[s]) <= startingTimeService[s2] + 1440*(1-x_bs[b, s2]);
      
  	forall(s in S, d in D)
    	forall(s2 in S: startingTimeService[s]  < startingTimeService[s2] )
        	x_ds[d, s] * (startingTimeService[s] + durationTimeService[s]) <= startingTimeService[s2] + 1440*(1-x_ds[d, s2]);
...
execute {
	...
	writeln("== Bus-Service Matrix == ");
 	for(var bus = 1; bus <= nBuses; bus++) {
  		for(var service = 1; service <= nServices; service++){
   			write(x_bs[bus][service] + " ");  
  		}
  		writeln(); 
 	}
 	writeln();
 
 	writeln("== Driver-Service Matrix == ");
 	for(var driver = 1; driver <= nDrivers; driver++) {
  		for(var service = 1; service <= nServices; service++){
   			write(x_ds[driver][service] + " ");  
  		}
  		writeln();
 	}
} 
